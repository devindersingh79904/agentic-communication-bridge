...
def speed_up_and_mock_tools():
    """Speeds up delays and mocks orchestrator tool dependencies for websocket tests."""
    with patch.object(config, "AGENT_STEP_DELAY_SECONDS", 0), \
         patch.object(config, "APPROVAL_TIMEOUT_SECONDS", 1), \
         patch("app.services.agent_orchestrator_service.research_tool", new_callable=AsyncMock) as m_res, \
...
def test_websocket_approval_flow(test_client):
    ...
    with test_client.websocket_connect("/v1/agent/connect") as ws:
        ...
        # 2. Receive status updates and approval required event
        events = []
        for _ in range(5):
            msg = ws.receive_json()
            events.append(msg)
        ...
        app_req = events[-1]
        assert app_req["draft_message"] == "Refined Draft"
        ...
        assert events_after[-1]["task_state"] == "SUCCESS"
